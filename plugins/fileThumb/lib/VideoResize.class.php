...
class videoResize {
	...
	public function run($path,$fileInfo,$plugin){
		...
		$args 		= '-c:a aac -preset medium -vf '.$quality.' -strict -2 -c:v libx264 1>'.escapeShell($logFile).' 2>&1';
		$script 	= $plugin->memLimitParam('ffmpeg', $command).' -y -i '.escapeShell($localFile).' '.$args.' '.escapeShell($tempPath);
